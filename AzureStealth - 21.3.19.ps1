    ...
    if (-not $UseCurrentCred) {
        ...
        if ($CloudShellMode) {
            try {
                ...
                Write-Host "Couldn't connect using the `"Connect-AzureADservice`" API call,`nThe tool will connect with `"Connect-AzureActiveDirectory `" call" -ForegroundColor Yellow
                $AzConnection = Connect-AzureActiveDirectory -AzContext $currentAzContext 
            }
        }
        else {
